...
func NewFileSaver(ctx context.Context, fs fs.FS, blobSaver *BlobSaver, pol chunker.Pol, workers int) *FileSaver {
	ch := make(chan saveFileJob, workers)

	s := &FileSaver{
		fs:           fs,
		blobSaver:    blobSaver,
		saveFilePool: NewBufferPool(ctx, 3*workers, chunker.MaxSize/4),
		pol:          pol,
		ch:           ch,
	}
...
func (s *FileSaver) Save(ctx context.Context, snPath string, file fs.File, fi os.FileInfo, f CompleteFunc) FutureFile {
	ch := make(chan saveFileResponse, 1)
	s.ch <- saveFileJob{snPath: snPath, file: file, fi: fi, complete: f, ch: ch}
...
func (s *FileSaver) saveFile(ctx context.Context, chnker *chunker.Chunker, snPath string, f fs.File, fi os.FileInfo) saveFileResponse {
	stats := ItemStats{}
...
func (s *FileSaver) worker(ctx context.Context, wg *sync.WaitGroup, jobs <-chan saveFileJob) {
	...
	for {
		...
		res := s.saveFile(ctx, chnker, job.snPath, job.file, job.fi)
		if job.complete != nil {
